              ...
              <div key={`${item.id}-${index}`} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex gap-4 items-center group hover:border-blue-100 transition-colors">
                ...
                <Link href={`/product/${item.id}`} className="flex items-center gap-4 flex-grow min-w-0">
                  ...
                  <div className="flex-grow min-w-0">
                    <h3 className="font-black text-slate-800 uppercase truncate text-sm mb-1 group-hover:text-blue-600 transition-colors">
                      {item.name}
                    </h3>
                ...
                {/* Кнопка видалення (залишається окремою) */}
                <button
                  onClick={() => removeFromCart(item.id)}
                  className="w-10 h-10 flex items-center justify-center text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all flex-shrink-0"
                  title="Видалити"
                >
