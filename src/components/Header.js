...
export default function Header() {
  ...
  const [allProducts, setAllProducts] = useState([]); // Стан для товарів з бази
    ...
    <header className="sticky top-0 z-[100] shadow-sm font-sans bg-white">
      ...
      {isMenuOpen && (
        <>
          ...
          <div className="absolute top-full left-0 w-full bg-white border-b border-gray-200 py-4 px-4 z-50 md:hidden animate-in slide-in-from-top duration-300 shadow-2xl rounded-b-[1.5rem]">

            {/* Навігація (Зменшені відступи) */}
            <nav className="flex flex-col mb-6">
              ...
              <a
                ...
                className="flex items-center gap-4 p-4 border-b border-slate-100 hover:bg-slate-50 transition-colors group"
              >
                ...
                <span className="font-bold text-sm text-slate-700">Каталог запчастин</span>
              </a>
              ...
              >
                ...
                <span className="font-bold text-sm text-slate-700">Оплата і доставка</span>
              </a>
              ...
              >
                ...
                <span className="font-bold text-sm text-slate-700">Гарантія</span>
              </a>
              ...
              >
                ...
                <span className="font-bold text-sm text-slate-700">Про нас</span>
              </a>

            </nav>

            {/* Контакти (Тільки іконки + Дзвінок) */}
            <div className="pt-4 border-t border-gray-100">

              {/* Ряд іконок месенджерів (Без тексту) */}
              <div className="flex justify-center gap-6 mb-4">
                <a href={`viber://chat?number=%2B${phoneNumber}`} className="w-12 h-12 flex items-center justify-center bg-slate-50 rounded-2xl text-[#7360f2] hover:bg-[#7360f2] hover:text-white transition-all shadow-sm">
                  <ViberIcon className="w-7 h-7" />
                </a>
                <a href={`https://t.me/+${phoneNumber}`} target="_blank" className="w-12 h-12 flex items-center justify-center bg-slate-50 rounded-2xl text-[#229ED9] hover:bg-[#229ED9] hover:text-white transition-all shadow-sm">
                  <TelegramIcon className="w-7 h-7" />
                </a>
                <a href={`https://wa.me/${phoneNumber}`} target="_blank" className="w-12 h-12 flex items-center justify-center bg-slate-50 rounded-2xl text-[#25D366] hover:bg-[#25D366] hover:text-white transition-all shadow-sm">
                  <WhatsAppIcon className="w-7 h-7" />
                </a>
              </div>

              {/* Кнопка подзвонити (Залишили як є) */}
              <a href={`tel:+${phoneNumber}`} className="w-full flex items-center justify-center gap-2 py-3.5 bg-slate-900 text-white rounded-xl font-bold text-sm shadow-lg shadow-slate-200 active:scale-95 transition-transform">
